    ...
    try {
        ...
        if (!data.exists) {
            ...
            if (!createReaderBtn) {
                ...
                btn.onclick = () => window.location.href = '/readers';
        ...
        document.getElementById('reader-name').textContent = `${data.reader.last_name} ${data.reader.first_name} ${data.reader.patronymic || ''}`.trim();
        ...
        if (createReaderBtn) createReaderBtn.remove();
    } catch (error) {
...
async function searchAvailableBooks() {
    const query = document.getElementById('book-name-search').value.trim();
    try {
        const response = await fetch(`/api/books/available?query=${encodeURIComponent(query)}`);
        const data = await response.json();
        if (!response.ok || data.error) throw new Error(data.error || 'Ошибка поиска книг');

        availableBooksCache = data.books || [];
        const select = document.getElementById('book-select');
        select.innerHTML = '<option value="">Выберите книгу из списка</option>';
        availableBooksCache.forEach(book => {
            const option = document.createElement('option');
            option.value = book.id;
            option.textContent = `${book.name} — ${book.author} (в наличии: ${book.quantity})`;
...
function selectBookFromList() {
    const selectedId = Number(document.getElementById('book-select').value);
    if (!selectedId) {
        currentBook = null;
        document.getElementById('book-info').style.display = 'none';
        return;
    }
    const book = availableBooksCache.find(b => b.id === selectedId);
    if (!book) return;

    currentBook = { id: book.id, title: book.name, author: book.author, year: '-', publishing_house: '-', quantity: book.quantity };
    ...
    document.getElementById('book-publishing_house').textContent = currentBook.publishing_house;
    document.getElementById('book-info').style.display = 'block';
}

async function issueBook() {
    if (!currentReader || !currentBook) {
        alert('Необходимо найти читателя и выбрать книгу из списка');
        return;
    }
    ...
    try {
        const response = await fetch('/api/book/issue', {
            ...
            body: JSON.stringify({ reader_id: currentReader.id, book_id: currentBook.id, issue_date: issueDate, return_date: returnDate })
        });
...
function resetIssueForm() {
    ...
    document.getElementById('book-name-search').value = '';
    document.getElementById('book-select').innerHTML = '<option value="">Выберите книгу из списка</option>';
    document.getElementById('book-info').style.display = 'none';
...
let standartRentalPeriod = 10;

async function loadSystemSettings() {
    try {
        const response = await fetch('/api/system/get');
        const data = await response.json();
        if (response.ok && data.success && data.system_settings) {
            standartRentalPeriod = data.system_settings.standart_rental_period || 10;
        }
    } catch (error) {
...
async function findReaderForReturn() {
    ...
    try {
        ...
        if (data.error || !data.exists) throw new Error(data.error || 'Читатель не найден');

        returnReader = data.reader;
        document.getElementById('return-reader-name').textContent = `${data.reader.last_name} ${data.reader.first_name} ${data.reader.patronymic || ''}`.trim();
...
async function findBookForReturn() {
    ...
    try {
        ...
        if (today > plannedDate) {
            ...
            document.getElementById('debt-message').textContent = `Просрочка: ${diffDays} дней (планировалось вернуть ${formatDate(data.planned_return_date)})`;
...
async function processReturn() {
    ...
    try {
        const response = await fetch('/api/book/return', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ record_id: returnBookRecord.record_id, actual_return_date: actualReturnDate })
        });
        const data = await response.json();
        if (data.error) throw new Error(data.error);
