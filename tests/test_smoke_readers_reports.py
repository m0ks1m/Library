...
class ReadersReportsSmokeTest(unittest.TestCase):
    ...
    def test_supplies_block_flow(self):
        ...
        available_books = self.client.get('/api/books/available?query=Война')
        self.assertEqual(available_books.status_code, 200)
        self.assertTrue(len(available_books.get_json()['books']) >= 1)
